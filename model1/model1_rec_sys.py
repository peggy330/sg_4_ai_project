...
for etf in etf_list:
    ...
    # 7일 후 가격의 비율 계산
    df_merged['seven_Day_Percent_Change'] = ((df_merged['Close_After_7_Days'] / df_merged['Close']) - 1) * 100

    # 필요한 컬럼만 유지
    df_final = df_merged[['Date', 'Close', 'seven_Day_Percent_Change']]
...
for name in tickers.keys():
    ...
    try:
        df = pd.read_csv(file_path)
        df['Date'] = pd.to_datetime(df['date'])
        df = df[['Date', 'seven_day_change', 'seven_day_volatility']]
...
for etf in etf_list:
    ...
    try:
        df = pd.read_csv(file_path)
        df['Date'] = pd.to_datetime(df['Date'])
        df = df[['Date', 'seven_Day_Percent_Change']]
        all_data[etf] = df
    except Exception as e:
...
input_columns = [col for col in merged_df.columns if 'seven_day_change' in col or 'seven_day_volatility' in col]
output_columns = [col for col in merged_df.columns if 'seven_Day_Percent_Change' in col]
...
# Define the input and output columns
input_columns = [col for col in recent_data.columns if 'seven_day_change' in col or 'seven_day_volatility' in col]
output_columns = [col for col in recent_data.columns if 'seven_Day_Percent_Change' in col]
...
"""
model1의 ETF 가격 예측 모델을 포트폴리오 추천시스템과 연결하는 과정
"""
etf_df = pd.DataFrame({})
for etf_price_file in os.listdir('peggy330/sg_4_ai_project/etf_data'):
  file_df = pd.read_csv(f'peggy330/sg_4_ai_project/etf_data/{etf_price_file}')
  etf_df = pd.concat([etf_df, file_df[:,'seven_Day_Percent_Change']], axis=1)
  etf_df.rename(columns={'seven_Day_Percent_Change':etf_price_file[:3})
